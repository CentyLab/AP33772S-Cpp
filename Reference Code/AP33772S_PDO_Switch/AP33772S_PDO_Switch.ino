...
void allpdo() {
  delay(10);
    //Request Maximum Current and Voltage of the selected PDO_INDEX
    for (int i = 0; i < 2; i++) {  // Loop through all possible PDO indices from 0 to 13
      // Check if both bytes are zero
      if (SRC_SPRandEPRpdoArray[i].byte0 == 0 && SRC_SPRandEPRpdoArray[i].byte1 == 0) {
        //return;  // If both bytes are zero, exit the function
      }else{
        writeBuf[0] = 0xFF;
        writeBuf[1] = 0x0F | (i+1 << 4);;
        i2c_write(SLAVE_ADDRESS, 0x31, writeBuf, 2);  // Write the data to the slave device
        while (1) {
          // Read the current state of GPIO 13
          currentState = digitalRead(13);
        ...
        String pdoType = (i+1 < 8) ? "SRC_SPR_PDO" : "SRC_EPR_PDO";
